...
// Обработчик для всех задач `getAllTasks`
...
// Обработчик для получения задачи по ID `postTask`
func postTask(w http.ResponseWriter, r *http.Request) {
	...
	// Проверка: если задача с таким ID уже есть
...
// Обработчик для получения задачи по ID `getTaskId`
func getTaskId(w http.ResponseWriter, r *http.Request) {
	...
	_, err = w.Write(resp)
	if err != nil {
		fmt.Printf("Ошибка при отправке ответа: %s\n", err)
	}
}
